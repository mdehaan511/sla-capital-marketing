...
const PRODUCTS = {
  dscr: {
    ...
    heroBody: (s) => `Long-term rental financing for investment properties across ${s.name} — qualified off the property's income, not your personal DTI. <strong>30-year fixed from 5.95%</strong>, up to <strong>80% LTV</strong>, and <strong>6 months of seasoning</strong> on cash-out refinances. SLA Capital has closed deals across every major ${s.name} metro.`,
    ...
    schemaDesc: (s) => `30-year fixed DSCR rental loan for investment properties in ${s.name}. Rates from 5.95%, up to 80% LTV, 6-month cash-out seasoning. Serving every major ${s.name} metro.`,
  },
...
function otherProductsCrossLinks(currentProductKey, state) {
  ...
  const cards = others.map(([key, p]) => {
    ...
    const oneLiner = key === 'dscr'
      ? `Long-term rental financing for ${state.name} investors. Rates from 5.95%, up to 80% LTV, 6-month seasoning on cash-outs.`
      : key === 'fixNFlip'
