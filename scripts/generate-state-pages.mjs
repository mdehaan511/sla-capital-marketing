...
const PRODUCTS = {
  dscr: {
    ...
    parentUrl: '/rental/',
    heroPromise: '30-Year Fixed from 6.90%',
    heroBody: (s) => `Long-term rental financing for investment properties across ${s.name} — qualified off the property's income, not your personal DTI. <strong>30-year fixed from 6.90%</strong>, up to <strong>80% LTV</strong>, and <strong>no seasoning</strong> on cash-out refinances. SLA Capital has closed deals across every major ${s.name} metro.`,
    ...
    specs: [
      { chip: 'Pricing',       h: 'Rates from 6.90%',  p: 'Priced off the 5-year Treasury. Real quote in minutes via our loan sizer — no bait-and-switch at close.' },
    ...
    faq: (s) => [
      { q: `Does SLA Capital lend on rental properties in ${s.name}?`, a: `Yes. SLA Capital funds DSCR rental loans throughout ${s.name} — ${s.metros.slice(0, 5).map(m => m.name).join(', ')}, and every other ${s.abbr} metro. Rates from <strong>6.90%</strong>, up to 80% LTV.` },
      { q: `What DSCR loan rates are available in ${s.name}?`, a: `${s.name} DSCR loans start at <strong>6.90%</strong> on a 30-year fixed structure. Priced off the 5-year Treasury with transparent up-front pricing — the rate you're quoted is the rate you close at.` },
    ...
    schemaDesc: (s) => `30-year fixed DSCR rental loan for investment properties in ${s.name}. Rates from 6.90%, up to 80% LTV, no cash-out seasoning. Serving every major ${s.name} metro.`,
  },
...
function otherProductsCrossLinks(currentProductKey, state) {
  ...
  const cards = others.map(([key, p]) => {
    ...
    const oneLiner = key === 'dscr'
      ? `Long-term rental financing for ${state.name} investors. Rates from 6.90%, up to 80% LTV, no seasoning on cash-outs.`
      : key === 'fixNFlip'
