  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: `DSCR rental loans from ${products.find(p => p.slug === 'rental')?.rateFrom || '6.80%'} (30-year fixed). Fix & Flip bridge from ${products.find(p => p.slug === 'fix-n-flip')?.rateFrom || '9.5%'} (6–18 months). New Construction from ${products.find(p => p.slug === 'new-construction')?.rateFrom || '10%'} (18 or 24 months). Rates effective ${fmtEffective(eff)} and priced off the 5-year Treasury.`,
      },
    },
    {
      '@type': 'Question',
...
<head>
  ...
  <meta name="description" content="SLA Capital current investor loan rates effective ${fmtEffective(eff)}. DSCR from ${esc(products.find(p => p.slug === 'rental')?.rateFrom || '6.80%')}. Fix &amp; Flip from ${esc(products.find(p => p.slug === 'fix-n-flip')?.rateFrom || '9.5%')}. New Construction from ${esc(products.find(p => p.slug === 'new-construction')?.rateFrom || '10%')}. The rate you're quoted is the rate you close at." />
