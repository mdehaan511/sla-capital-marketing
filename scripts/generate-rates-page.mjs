...
const tableRows = products.map(p => `
        <tr>
          ...
          <td style="padding:14px 16px;border-top:1px solid var(--border);font-family:var(--font-display);font-size:20px;color:var(--primary);font-weight:600">from ${esc(p.rateFrom)}${p.rateNote && p.rateNote.startsWith('*') ? '*' : ''}${p.rateNote ? `<div style="font-family:var(--font-body);font-size:11px;font-weight:400;color:var(--muted);line-height:1.4">${esc(p.rateNote)}</div>` : ''}</td>
...
const detailCards = products.map(p => `
      <div class="card">
        ...
        <div style="font-family:var(--font-display);font-size:36px;font-weight:700;color:var(--primary);line-height:1;margin:14px 0 4px">from ${esc(p.rateFrom)}${p.rateNote && p.rateNote.startsWith('*') ? '*' : ''}</div>${p.rateNote ? `
  ...
  mainEntity: [
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: `DSCR rental loans from ${products.find(p => p.slug === 'rental')?.rateFrom || '6.97%'} (30-year fixed). Fix & Flip bridge from ${products.find(p => p.slug === 'fix-n-flip')?.rateFrom || '9.5%'} (6–18 months). New Construction from ${products.find(p => p.slug === 'new-construction')?.rateFrom || '10%'} (18 or 24 months). Rates effective ${fmtEffective(eff)} and priced off the 5-year Treasury.`,
      },
    },
    {
      '@type': 'Question',
...
<head>
  ...
  <meta name="description" content="SLA Capital current investor loan rates effective ${fmtEffective(eff)}. DSCR from ${esc(products.find(p => p.slug === 'rental')?.rateFrom || '6.97%')}. Fix &amp; Flip from ${esc(products.find(p => p.slug === 'fix-n-flip')?.rateFrom || '9.5%')}. New Construction from ${esc(products.find(p => p.slug === 'new-construction')?.rateFrom || '10%')}. The rate you're quoted is the rate you close at." />
  ...
  <footer class="site-footer">
    <div class="container">
      ...
      <div class="footer-bottom">
        <div>© 2026 SLA Capital · Sir Lends A Lot LLC · All rights reserved.<br><span style="font-size:12px;color:rgba(255,255,255,0.55)">Sir Lends A Lot LLC · 707 W Main Ave #31, Spokane, WA 99201 · NMLS ID #2863552 · <a href="https://www.nmlsconsumeraccess.org/EntityDetails.aspx/COMPANY/2863552" target="_blank" rel="noopener" style="color:rgba(255,255,255,0.7)">NMLS Consumer Access</a> · Idaho Mortgage Broker/Lender License #MBL-2082863552 · Certified Member — <a href="https://aaplonline.com/" target="_blank" rel="noopener" style="color:rgba(255,255,255,0.7)">American Association of Private Lenders (AAPL)</a></span></div>
